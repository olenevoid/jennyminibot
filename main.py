...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    bot = context.bot
    log.info(f'User "{update.message.from_user.username}" in chat "{update.message.chat_id}" sent a message')
    await message_manager.process_message(update, bot, parse_mode='MarkdownV2')


async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):    
    await message_manager.send_reply(update, text=f'This is a Gemini bot "{AI_MODEL}"', parse_mode='MarkdownV2')


async def request_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    request = update.message.text
    response = await gemini.request_async(request)
    await message_manager.send_reply(update, text=response, parse_mode='MarkdownV2')
...
def launch_bot():
    app = Application.builder().token(TG_KEY).read_timeout(READ_TIMEOUT).write_timeout(WRITE_TIMEOUT).build()
